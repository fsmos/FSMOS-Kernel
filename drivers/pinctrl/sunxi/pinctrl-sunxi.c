...
{
	struct sunxi_pinctrl *pctl = irq_data_get_irq_chip_data(d);
	u32 status_reg = sunxi_irq_status_reg(d->hwirq,pctl->desc->irq_bank_base);
		       ...
		       const struct sunxi_pinctrl_desc *desc)
{
	...
	for (i = 0; i < pctl->desc->irq_banks; i++) {
		...
		writel(0, pctl->membase + sunxi_irq_ctrl_reg_from_bank(i,pctl->desc->irq_bank_base));
		writel(0xffffffff,
			pctl->membase + sunxi_irq_status_reg_from_bank(i,pctl->desc->irq_bank_base));
